...
class CustomerWallet extends Model
{
    ...
    public function requestOverdraft()
    {
        ...
        if($info['result'] == 4) {
            return [
                'access' => true,
                'value' => $info['amount'] > $this->getLimitOverdraftByType() ? eur($this->getLimitOverdraftByType()) : eur(ceil($info['result']/100) * 100),
                'taux' => $taux." %"
            ];
        } else {
            return [
    ...
    private function calcOverdraftPro()
    {
        ...
        $result = $this->customer->wallets()->where('type', 'compte')->sum('balance_actual') + $this->customer->business->ca;
