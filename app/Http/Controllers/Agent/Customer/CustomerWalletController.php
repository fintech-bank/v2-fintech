...
class CustomerWalletController extends Controller
{
    ...
    public function postCaution($number_account, Request $request)
    {
        ...
        $caution = CustomerPretCaution::create([
            ...
            'civility' => $request->get('civility') ? $request->get('civility') : '',
            ...
            'password' => null,
            'num_cni' => $request->get('num_cni') ? $request->get('num_cni') : '',
            'date_naissance' => $request->get('date_naissance') ? Carbon::createFromTimestamp(strtotime($request->get('date_naissance')))->toDateTime() : '',
            'country_naissance' => $request->get('country_naissance') ? $request->get('country_naissance') : '',
            'dep_naissance' => $request->get('dep_naissance') ? $request->get('dep_naissance') : '',
            'ville_naissance' => $request->get('city_naissance') ? $request->get('city_naissance') : '',
            'persona_reference_id' => 'caution_'.$request->get('type').'_'.now()->format('dmY'),
            'type_structure' => $request->get('type_structure') ? $request->get('type_structure') : '',
            'siret' => $request->get('siret') ? $request->get('siret') : '',
