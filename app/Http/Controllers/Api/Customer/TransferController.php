    ...
    private function immediatTransfer(CustomerWallet $wallet,CustomerBeneficiaire $beneficiaire, Request $request)
    {
        if($wallet->solde_remaining <= 0) {
            $transfer = $wallet->transfers()->create([
                ...
                'reference' => $request->get('reference') != null ?? generateReference(),
                'reason' => $request->get('reason') != null ?? "Virement vers le compte ".CustomerTransferHelper::getNameBeneficiaire($beneficiaire),
                'type' => 'immediat',
                'transfer_date' => now(),
                'status' => 'pending',
